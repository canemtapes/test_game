...
//--------------------------------------------------------------
void ofApp::setup() {
	ofSetWindowShape(960, 540); //Tama隳 de la ventana por default
	...
	message_text = "·resiona ENTER para jugar!";
...
void ofApp::update() {
	...
	if (!pause) {
		...
		//Colisi鏮 personaje y personas
		playerCrash();

		//Colisi鏮 personaje y fuel
		fuelCollector();
...
void ofApp::keyPressed(int key) {
	...
	if (key == OF_KEY_RETURN) {
		...
		pause = !pause;
		//Volvemos a posicionar al jugador en la posici鏮 de origen.
		player.x = width / 2;
		player.y = height - 200;
...
void ofApp::timer() {
	...
	if (timeBarWidth <= 0.0f)
	{
		...
		//Se muestra este mensaje en esta posici鏮.
		message_text = "﹗e quedaste sin tiempo! ‥a nave ha despegado!";
		messageTextPos.x = width / 2 - (messageFT.stringWidth(message_text) / 2);
	}
}
...
void ofApp::scenery() {
	...
	if (!jetOn) {
		...
		jetPos.y += speed; //la posicion aumenta en relaci鏮 a la velocidad
...
//Colisi鏮 entre personaje y personas --------------------------
...
//Colisi鏮 entre personaje y combustible -----------------------
void ofApp::fuelCollector() {
	...
	//PP agarra combustible ecol鏬ico
	for (int i = 0; i < peopleMax; i++) {
